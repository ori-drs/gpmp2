function execute_update_case_reinit = case7(datasets, init_values, problem_setup)
...
%     update_timings.factors_updated = cell(1, problem_setup.total_time_step+1);
    ...
    execute_update_case_reinit.gp_cost_evolution = zeros(1, problem_setup.total_time_step+1);

    for i = 0:problem_setup.total_time_step
        disp("Case7: Execute and update sdf using reinitialisation... step: " + num2str(i));
...
%                 update_timings.factors_updated{i+1} = vertcat(update_timings.factors_updated{i+1},all_obs_fact_indices{j+1});
        ...
        execute_update_case_reinit.gp_cost_evolution(i+1) = gp_cost;
    end
    
    execute_update_case_reinit.final_result = result;
    execute_update_case_reinit.results = results;
    execute_update_case_reinit.graph_build_t = graph_build_t;
    execute_update_case_reinit.update_timings = update_timings;
    execute_update_case_reinit.all_obs_fact_indices = all_obs_fact_indices;
    execute_update_case_reinit.obs_fact_indices = obs_fact_indices;
    execute_update_case_reinit.gp_fact_indices = gp_fact_indices;
