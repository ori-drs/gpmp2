...
void Arm::forwardKinematics(
  ...
  // variables
  vector<Matrix4, Eigen::aligned_allocator<Matrix4>> H(dof());
  vector<Matrix4, Eigen::aligned_allocator<Matrix4>> Ho(dof()+1); // start from 1
  vector<Matrix, Eigen::aligned_allocator<Matrix>> J;
  if (jv) J.assign(dof(), Matrix::Zero(3, dof()));
  // vars cached for calculate output Jacobians
  vector<Matrix4, Eigen::aligned_allocator<Matrix4>> dH(dof());
  vector<Matrix4, Eigen::aligned_allocator<Matrix4>> Hoinv(dof()+1); // start from 1
  ...
  vector<vector<Matrix4, Eigen::aligned_allocator<Matrix4>> > dHo_dq(dof(), vector<Matrix4, Eigen::aligned_allocator<Matrix4>>(dof()));
