/**
 *  @file  DynamicObstacleSDFFactor.h
...
class DynamicObstacleSDFFactor: public gtsam::NoiseModelFactor1<typename ROBOT::Pose> {
...
private:
  // typedefs
  typedef DynamicObstacleSDFFactor This;
...
public:
  ...
  DynamicObstacleSDFFactor() : robot_(Robot()), sdf_(SignedDistanceField()) {}
   ...
   */
  DynamicObstacleSDFFactor(gtsam::Key poseKey, const Robot& robot,
      SignedDistanceField& sdf, double cost_sigma, double epsilon) :
        Base(gtsam::noiseModel::Isotropic::Sigma(robot.nr_body_spheres(), cost_sigma), poseKey),
        epsilon_(epsilon), robot_(robot), sdf_(sdf) {}

  virtual ~DynamicObstacleSDFFactor() {}
  ...
  void changeSDFData(SignedDistanceField& new_sdf) {
    sdf_.replaceSDFData(new_sdf);
  }
  ...
  void print(const std::string& s="", const gtsam::KeyFormatter& keyFormatter = gtsam::DefaultKeyFormatter) const {
    std::cout << s << "DynamicObstacleSDFFactor :" << std::endl;
...
#include <gpmp2/obstacle/DynamicObstacleSDFFactor-inl.h>
